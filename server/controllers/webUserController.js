        ...
        WebUser.findOne({ email: email })
            .then(data => {
                if (!data) {
                    ...
                    let pas = Math.floor(Math.random() * 10000);
data.forgotpassword=pas;
                    data.save();
                    changePassword(data.email, pas)
    }})
...
function changePassword(email, pas) {
    const resetLink = `http://localhost:3000/changepassword?userId=${user._id}&token=${pas}`;
